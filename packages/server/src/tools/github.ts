...
function parseGitHubRepoUrl(input: string) {
  ...
  if (rawUrl.startsWith('git@github.com:')) {
    const [owner, repo] = rawUrl.replace('git@github.com:', '').split('/').filter(Boolean)
  ...
  async run(args) {
    ...
    if (!ref) {
      return JSON.stringify({
        error: '缺少有效的仓库定位参数：请同时提供 owner 和 repo，或提供完整的 url。',
      })
    }
    ...
    return JSON.stringify(
      {
        ...
        url: toStringValue(data.html_url, `https://github.com/${ref.owner}/${ref.repo}`),
